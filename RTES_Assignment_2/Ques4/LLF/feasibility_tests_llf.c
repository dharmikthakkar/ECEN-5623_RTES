...
int llf_util_bound_sched_test(U32_T numServices, U32_T period[], U32_T wcet[], U32_T deadline[]);
/*EDF Timing Analysis Test */
int llf_timing_analysis_sched_feasibility_test(U32_T numServices, U32_T period[], U32_T wcet[], U32_T deadline[]);
...
{ 
    ...
    printf("******** LLF Utility Bound Analysis ********\n");
    
    if(llf_util_bound_sched_test(numServices, ex_period, ex_wcet, ex_period) == TRUE)
        printf("FEASIBLE\n");
    else
    {
        printf("INFEASIBLE\n");
    ...
    printf("************ LLF Timing Analysis ************\n");
    
    if(llf_timing_analysis_sched_feasibility_test(numServices, ex_period, ex_wcet, ex_period) == TRUE)
        printf("FEASIBLE\n");
    else
        printf("INFEASIBLE\n");
...
void Assign_Priorities(void)
{
     ...
     for (i = 0; i < size; i++)
     {
         if (time_diff[i] == 100)
            ...
            laxity[i] = time_diff[i] - ex_wcet[i];    
     }

     for (i = 0; i < size; i++)
         *(sorted_laxity+i) = *(laxity+i);
