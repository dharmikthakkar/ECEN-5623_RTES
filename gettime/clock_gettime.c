...
{
	uint64_t diff;
	struct timespec start, end, set_time;
	int i;
...
//	clock_gettime(CLOCK_REALTIME, &set_time);
//	printf("OLD time: %d seconds and %d nanoseconds\n", set_time.tv_sec, set_time.tv_nsec);

	set_time.tv_sec=60;
	set_time.tv_nsec = 500;
	if(clock_settime(CLOCK_REALTIME, &set_time) == -1){
		printf("set time error\n");
	}
	clock_gettime(CLOCK_REALTIME, &set_time);
	printf("New set time: %d seconds and %d nanoseconds\n", set_time.tv_sec, set_time.tv_nsec);
